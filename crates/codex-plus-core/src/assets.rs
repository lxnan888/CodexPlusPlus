...
const COMPOSER_BLUR_FIX_SCRIPT: &str =
    include_str!("../../../assets/inject/composer-blur-fix.js");
...
pub fn injection_script_with_settings(helper_port: u16, settings: &BackendSettings) -> String {
    ...
    format!(
        ...
        serde_json::to_string(&composer_blur_fix_enabled)
            .expect("composer blur fix config should serialize"),
        renderer_script(),
        stepwise_script(),
        dream_skin_target_runtime,
        if composer_blur_fix_enabled {
            COMPOSER_BLUR_FIX_SCRIPT
        } else {
            ""
        },
    )
